...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.conv = nn.Conv1d(input_size, hidden_size, kernel_size=(1,)).double()

    def forward(self, input_vec):
        output = self.conv(input_vec.double())
...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden):
        ...
        # Broadcast some dimensions so we can do batch-matrix-multiply（batch-size个样本一起训练）
...
class Pointer(nn.Module):
    ...
    def __init__(self, hidden_size, num_layers=1, dropout=0.2):
        ...
        self.gru = nn.GRU(hidden_size, hidden_size, num_layers,
                          batch_first=True,
                          dropout=dropout if num_layers > 1 else 0).double()
        self.encoder_attn = Attention(hidden_size).double()

        self.drop_rnn = nn.Dropout(p=dropout).double()
        self.drop_hh = nn.Dropout(p=dropout).double()
...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size,
        ...
        self.static_encoder = Encoder(static_size, hidden_size).double()
        self.dynamic_encoder = Encoder(dynamic_size, hidden_size).double()
        self.decoder = Encoder(static_size, hidden_size).double()
        self.pointer = Pointer(hidden_size, num_layers, dropout).double()
